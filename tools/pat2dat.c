...
int opt_compression = 0;
int opt_overwrite = FALSE;
int opt_8bit = FALSE;
int opt_verbose = FALSE;
int opt_veryverbose = FALSE;
int opt_bank = 0;
int opt_drum_bank = 0;
char *opt_datafile = NULL;
char *opt_configfile = NULL;
...
int need_patches[128], need_drums[128];
...
void scan_patches(MIDI *midi, char *patches, char *drums)
{
   unsigned char *p, *end;
...
void read_midi(char *filename, int attrib, int param)
{
...
void mem_modify32(int pos, int val)
{
   mem[pos] = val & 0xFF;
...
void mem_write_block(void *data, int size)
{
...
void mem_write_file(PACKFILE *f, int size)
{
   if (mem_size+size > mem_alloced) {
      mem_alloced = (mem_alloced + size + 4095) & ~4095;
      mem = realloc(mem, mem_alloced);
...
void *grab_patch(char *filename, long *size, int x, int y, int w, int h, int depth)
{
   PACKFILE *f;
...
void apply_generator(sfGenList *g, int preset)
{
   switch (g->sfGenOper) {
...
int key2freq(int note, int cents)
{
...
void grab_soundfont_sample(char *name)
{
...
int grab_soundfont(int num, int drum, char *name)
{
...
void calc_end(RIFF_CHUNK *chunk, FILE *f)
{
...
void print_sf_string(FILE *f, char *title)
{
   char buf[256];
   char ch;
   int i = 0;
...
int main(int argc, char *argv[])
{
   ...
   for (c=0; object_info[c].type != DAT_END; c++)
      if (object_info[c].type == DAT_PATCH)
	 object_info[c].grab = grab_patch;
